---
title: "Miriam Lerma"
image: "images/LermaCircular.jpg"
links:
  - label: '<i class="fab fa-mastodon"></i> Mastodon'
    url: "https://tinyurl.com/47d4fvnx"
  - label: '<i class="fab fa-github"></i> GitHub'
    url: "https://github.com/MiriamLL"
  - label: '<i class="fas fa-envelope"></i> Email'
    url: "mailto:miriamjlerma@gmail.com"
twitter:
  creator: "@MiriamLermaL"
  site: "@MiriamLermaL"
output:
  postcards::trestles
site: distill::distill_website
---

```{r setup, message=FALSE, warning=FALSE, include=FALSE}
library(metathis)
meta() %>%
  meta_description(
    "Mi sitio personal"
  ) %>%
  meta_name("github-repo" = "https://github.com/MiriamLL/miriamlerma_") %>%
  meta_viewport() %>%
  meta_social(
    title = "Miriam Lerma",
    url = "www.miriam-lerma.com",
    og_type = "website",
    og_author = c("Miriam Lerma"),
    twitter_card_type = "summary",
    twitter_creator = "@MiriamLermaL"
  )
```

```{r load_packages, message=FALSE, warning=FALSE, include=FALSE}
library(fontawesome)
```

### `r fa("chevron-circle-down", fill = "black")` About  me

I am part of the Seevogelmonitoring team at the Dachverband Deutscher Avifaunisten [DDA](https://www.dda-web.de/). My work entitles analyzing spatial patterns of marine animals.

I am also collaborating on active projects between Germany ([FTZ](https://www.ftz.uni-kiel.de/de)), Mexico ([CICESE](https://www.cicese.edu.mx/)) and Chile ([UCN](https://www.ucn.cl/)). 
My focus is coastal and marine animals, in particular shorebirds and seabirds.  My research interests include conservation, ecology, physiology, and ecotoxicology. You can download my cv [here](https://raw.githubusercontent.com/MiriamLL/Curriculum/master/CV/CV-Lerma.pdf). 

I am passionate about nature, biology, and outdoor activities (hiking, kayaking, snorkelling).   

If you are interested in a collaboration, or have any questions, please feel free to email me. 

### `r fa("envelope", fill = "black")` Contact
**Dr. rer. nat. Miriam Lerma** <br>
Email: miriamjlerma@gmail.com