...
Assign to the data fram
```{r}
colonies_sub$distances<-distances$value
```
...
Use the function **annotation_scale** 
```{r, fig.height= 7}
base_plot<-base_plot+
  annotation_scale(location = "tr",bar_cols = c("#e0fbfc", "#3d5a80"),text_col = '#3d5a80')
base_plot
```
...
Using the package [nngeo](https://cran.r-project.org/web/packages/nngeo/index.html)
