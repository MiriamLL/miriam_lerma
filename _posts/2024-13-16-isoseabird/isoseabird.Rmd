...
title: "isoseabird"
description: |
  A package with stable isotope data from sulids.  | Un paquete con datos de isotopos estables de bobos. 
preview: isoseabirdhex.png
author:
  - name: Miriam Lerma
date: 2023-04-16
output:
...
knitr::include_graphics("isoseabirdhex.png")
```
...
icon_link(icon = "fas fa-external-link-alt",
          text = "Github",
          url = "https://github.com/MiriamLL/isoseabird")
```
...
```{r}
Dactylatra_clean <- Sula_data %>%
  filter(Species == 'Dactylatra' & Nitrogeno > 16)
```

```{r}
Sula_clean<-Sula_data %>%
  filter(Species != 'Dactylatra')
```

```{r}
Sula_df<-rbind(Dactylatra_clean,Sula_clean)
```
...
Please refer to [Turner et al. 2010](https://esapubs.org/archive/ecol/E091/157/) and find the original script [here](https://esapubs.org/archive/ecol/E091/157/suppl-1.htm)

### organize_for_Turner

```{r}
SIA_Turner<-organize_for_Turner(SIA_samples=Sula_df,
                                column_C='Carbon',
                                column_N='Nitrogeno',
...
plot_centroids(SIA_samples=Sula_df,
               column_C='Carbon',
               column_N='Nitrogeno',
               column_group='Species',
               group1="SulaSula",
...
```{r}
SIA_group<-as.factor(Sula_df[,3]) #Designate groups
```
...
```{r}
SIBER_SIA<-organize_for_SIBER(SIA_samples=Sula_df,
                              column_C='Carbon',
                              column_N='Nitrogeno',
                              column_group='Species')
```
...
```{r, fig.height=2}
SIA_plot<-ggplot_ellipse(SIA_samples=Sula_df,
                         column_C='Carbon',
                         column_N='Nitrogeno',
                         column_group='Species')
SIA_plot
