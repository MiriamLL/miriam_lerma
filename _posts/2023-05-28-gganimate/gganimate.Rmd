...
```{r}
Example$date_diff <- as.Date(as.character(Example$tStamp), format="%Y-%m-%d")-as.Date(as.character('2017-11-02'), format="%Y-%m-%d")
```
...
Directory<-here('_posts/2023-02-28-gganimate')
MyShapefileName<-"/America.shp"
```
...
```{r}
ggplot(data=Example)  + 
  geom_sf(data = Country, colour = 'black', fill = '#9a8c98')+
  geom_point(data=Example,aes(x = Longitude, y=Latitude, colour = IDs))+
...
I use the package **av** and the package **animation** to export
...
To export, I create my plot as an object
...
I transform my plot from gif to mp4
...
[Step by step with Katehrine Goode](https://goodekat.github.io/presentations/2019-isugg-gganimate-spooky/slides.html#1)
