...
title: "larus"
description: |
  Functions to work with GSM data. 
preview: logo_larus_png.png
author:
  - name: Miriam Lerma
date: 2025-03-07
output:
...
knitr::include_graphics("https://github.com/MiriamLL/miriamlerma_/blob/main/_posts/2025-07-03-larus/logo_larus_png.png?raw=true")#man/figures/logo.png
```
...
plot_trips(my_locs=Path_trips %>%
  dplyr::filter(trip_number=='trip_00002'),my_central_location=nest_central_location)+
...
Path_04speed %>%
  ...
  relocate(trip_number,speed_if_directly_flying)
```

In case there is a trip exceeding speed:

```{r}
exceeds_speed<-'trip_00070'
```
